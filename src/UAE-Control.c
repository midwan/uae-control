/***********************************************************
*                                                          *
* UAE-Control - Emulator control from inside the emulation *
* (c) 1996 Tauno Taipaleenmaki <tataipal@raita.oulu.fi>    *
* (c) 1998, 2014 Cloanto <http://cloanto.com>              *
* (c) 2020 Dimitris Panokostas <midwan@gmail.com           *
*                                                          *
* Version 1.3                                              *
* Can be compiled with SAS/C, VBCC and GCC                 *
*                                                          *
* Requires Amiga OS 2.0 or higher. Users with Amiga OS 1.3 *
* or below should use the "uaectrl" command line version.  *
*                                                          *
***********************************************************/

#define VERSION_STRING "\0$VER: UAE-Control 1.3"
#define VERSION_SHELL " UAE-Control v. 1.3\n � 1996 Tauno Taipaleenmaki, � 1998, 2014 Cloanto, 2020 Dimitris Panokostas\n\n"
...
int main()
{
	...
	if (!InitUAEResource())
	{
		quit_program(2, "UAEResource not found!");
		return(2);
	}
				...
				case IDCMP_GADGETUP:
					switch( msgID )
					{
						...
						case GAD_DEBUG:
							DebugFunc();
							break;
						case GAD_EXECUTE_HOST:
							strcpy(buf, "");
							if (get_string(buf, 255))
							{
								strcat(buf, " &");
								ExecuteOnHost((UBYTE*)&buf);
							}
							break;
						default:
							break;
...
int setup_window(void)
{
	...
	ng.ng_TopEdge = offy + 124;
	ng.ng_GadgetText = (UBYTE *)"Run on Host";
	ng.ng_GadgetID = GAD_EXECUTE_HOST;
	g = CreateGadget(BUTTON_KIND, g, &ng, TAG_DONE);
